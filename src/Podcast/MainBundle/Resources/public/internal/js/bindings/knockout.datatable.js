...
ko.bindingHandlers['dataTable'] = {
    ...
    'init': function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
        ...
        //options.sDom = "<'row-fluid'<'span12'<'well well-small clearfix'lf>>r>t<'row-fluid'<'span6'i><'span6'p>>";
        options.sDom = '<""l>t<"F"fp>';
        options.bJQueryUI = true;
        options.oLanguage = {
            "sEmptyTable": "No episodes"
        };
        ...
        if (binding.rowTemplate && binding.rowTemplate != '') {
            ...
            options.fnRowCallback = cog.utils.intercept(options.fnRowCallback || function (row) {
                ...
                var isPlaying = soundManager.getSoundById(data.id);

                if(isPlaying && (isPlaying.playState === 1)) {
                    $(row).addClass('info');
                } else {
            ...
            options.fnRowCallback = cog.utils.intercept(options.fnRowCallback || function (row) {
                ...
                ko.utils.arrayForEach(columns, function (column) {
                    ...
                    ko.applyBindingsToNode(newCell[0], {
                        text: accesor
                    }, bindingContext.createChildContext(srcData));
                });
        ...
        $(document).trigger(ko.bindingHandlers.dataTable._onInitialisingEventName, {
            options: options
        });
